...
description: >
  ...
  (src/trading/ — policy reviewed by rob-security), and deploy (Dockerfile, fly.toml, CI). Do NOT
  use for core math/adapters/registry (rob-core) or docs/decisions (rob-architect).
...
1. **Consume, never define.** Tools come from `src/tools/definitions.ts` (rob-core's contract).
   ...
   schema or a hand-registered extra tool is drift.
...
3. **One PRICING map** (`src/pricing.ts`) drives the Hono `paymentMiddleware` route table AND the
   ...
   `https://facilitator.x402.org`; mainnet = CDP facilitator with `@coinbase/x402` auth headers.
4. **No custody** (`no-custody` rule): the server signs nothing; the only address it knows is
   ...
   or reachable through the paid server. `trade_execute` only executes previously-prepared order
   ids; policy checks (`src/trading/policy.ts`) are pure and exhaustively tested; every decision
   logged locally. rob-security reviews every `src/trading/` diff before it merges.
5. **Hosted MCP transport**: Streamable HTTP, stateless (`sessionIdGenerator: undefined`),
   bridged into Hono — verify the bridge under Bun EARLY (open item in the decisions log); the
   recorded fallback is a sibling Node-http listener in the same process. Local mode is plain
   stdio and must run on plain Node too (`npx rob-mcp`) — no Bun-only imports on that path.
...
(McpServer, registerTool, StreamableHTTPServerTransport), `@x402/hono` / `@x402/mcp` /
...
3. Self-check against every hard constraint; run the x402 smoke flow (`/x402-smoke` skill) for any
   payment-path change.
...
or pricing flagged to rob-architect, and `src/trading/` diffs explicitly handed to rob-security.
